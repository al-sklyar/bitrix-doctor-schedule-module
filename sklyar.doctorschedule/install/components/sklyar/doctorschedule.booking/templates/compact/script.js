(function () {
    function initBookingComponent(rootNode) {
        ...
        function renderSelectControl(container, config) {
            var options = Array.isArray(config.options) ? config.options : [];
            var selectedValue = config.selectedValue === null || typeof config.selectedValue === 'undefined'
                ? ''
                : String(config.selectedValue);
            var html = '<select class="skds-booking__select" data-role="' + escapeHtml(config.role) + '"';

            if (options.length === 0) {
                html += ' disabled>';
                html += '<option value="">' + escapeHtml(config.emptyLabel || getMessage('compactNoOptions', '')) + '</option>';
                html += '</select>';
        ...
        function renderSpecializations(usedSpecializations) {
            renderSelectControl(refs.specializationContainer, {
                role: 'select-specialization',
                selectedValue: state.selectedSpecializationId,
                emptyLabel: getMessage('compactNoOptions', ''),
                options: usedSpecializations.map(function (specialization) {
                    return {
        ...
        function renderDoctors(availableDoctors) {
            renderSelectControl(refs.doctorContainer, {
                role: 'select-doctor',
                selectedValue: state.selectedDoctorId,
                emptyLabel: getMessage('noDoctors', ''),
                options: availableDoctors.map(function (doctor) {
                    var specializationNames = getSpecializationNameList(doctor);
                    var label = doctor.name;

                    if (specializationNames.length > 0) {
                        label += ' - ' + specializationNames.join(', ');
                    }
        ...
        function renderAppointmentTypes(availableAppointmentTypes) {
            renderSelectControl(refs.appointmentTypeContainer, {
                role: 'select-appointment-type',
                selectedValue: state.selectedAppointmentTypeId,
                emptyLabel: getMessage('compactNoOptions', ''),
                options: availableAppointmentTypes.map(function (appointmentType) {
                    return {
        ...
        function renderServices(availableServices) {
            renderSelectControl(refs.serviceContainer, {
                role: 'select-service',
                selectedValue: state.selectedServiceId,
                emptyLabel: getMessage('noServices', ''),
                options: availableServices.map(function (service) {
                    ...
                    if (servicePrice) {
                        label += ' - ' + formatPrice(servicePrice.price, servicePrice.currency) +
                            ', ' + servicePrice.durationMinutes + ' ' + getMessage('minutesShort', '');
                    }
        ...
        function renderDates(availability) {
            renderSelectControl(refs.dateContainer, {
                role: 'select-date',
                selectedValue: state.selectedDate,
                emptyLabel: getMessage('noDates', ''),
                options: availability.map(function (dateItem) {
                    return {
                        value: dateItem.date,
                        label: dateItem.dayLabel + ', ' + dateItem.fullLabel
                    };
                })
            });
        }
        ...
        function renderLocation(availableLocations) {
            ...
            renderSelectControl(refs.locationContainer, {
                role: 'select-location',
                selectedValue: state.selectedLocationKey,
                emptyLabel: getMessage('locationAfterService', ''),
                options: availableLocations.map(function (location) {
        ...
        function renderSlots(availability) {
            ...
            renderSelectControl(refs.slotContainer, {
                role: 'select-slot',
                selectedValue: state.selectedSlotKey,
                emptyLabel: getMessage('selectDateFirst', ''),
                options: selectedDateItem ? selectedDateItem.slots.map(function (slot) {
                    return {
                        value: slot.key,
                        label: formatMinutes(slot.timeFromMinutes)
                    };
                }) : []
            });
        }
        ...
        rootNode.addEventListener('change', function (event) {
            ...
            if (target.matches('[data-role="select-specialization"]')) {
                clearFlashMessage();
                state.selectedSpecializationId = target.value === '' ? null : Number(target.value);
                state.selectedDoctorId = null;
                state.selectedAppointmentTypeId = null;
                state.selectedServiceId = null;
                state.selectedLocationKey = null;
            ...
            if (target.matches('[data-role="select-doctor"]')) {
                clearFlashMessage();
                state.selectedDoctorId = target.value === '' ? null : Number(target.value);
                state.selectedAppointmentTypeId = null;
                state.selectedServiceId = null;
                state.selectedLocationKey = null;
                state.selectedDate = null;
            ...
            if (target.matches('[data-role="select-appointment-type"]')) {
                clearFlashMessage();
                state.selectedAppointmentTypeId = target.value === '' ? null : Number(target.value);
                state.selectedServiceId = null;
                state.selectedLocationKey = null;
                state.selectedDate = null;
                state.selectedSlotKey = null;
            ...
            if (target.matches('[data-role="select-service"]')) {
                clearFlashMessage();
                state.selectedServiceId = target.value === '' ? null : Number(target.value);
                state.selectedLocationKey = null;
                state.selectedDate = null;
                state.selectedSlotKey = null;
                render();
            ...
            if (target.matches('[data-role="select-date"]')) {
                clearFlashMessage();
                state.selectedDate = target.value === '' ? null : target.value;
                state.selectedSlotKey = null;
                render();
            ...
            if (target.matches('[data-role="select-location"]')) {
                clearFlashMessage();
                state.selectedLocationKey = target.value === '' ? null : target.value;
                state.selectedDate = null;
                state.selectedSlotKey = null;
                render();
            ...
            if (target.matches('[data-role="select-slot"]')) {
                clearFlashMessage();
                state.selectedSlotKey = target.value === '' ? null : target.value;
                render();
            }
        });
